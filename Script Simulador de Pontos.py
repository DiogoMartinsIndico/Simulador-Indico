...
st.markdown("""
    ...
    /* Estilo para a seta e mensagem do modo dark */
    .dark-mode-tooltip {
        position: fixed;
        top: 15px;
        right: 80px;
    ...
    /* Estilo para destacar o menu de três pontos */
    .stApp > header [data-testid="stToolbar"] {
        position: relative;
    }
    
    .stApp > header [data-testid="stToolbar"]::before {
        content: '';
        position: absolute;
        top: -5px;
        right: -5px;
...
# Adicionar a mensagem de tooltip para modo dark
st.markdown("""
    <div class="dark-mode-tooltip">
        Altere aqui para modo dark para melhor visualização
    </div>
    
    <script>
    // Script para remover o tooltip após alguns segundos
    setTimeout(function() {
        const tooltip = document.querySelector('.dark-mode-tooltip');
        if (tooltip) {
            tooltip.style.animation = 'fadeOut 1s ease-out forwards';
        }
    }, 8000);
